        ...
        <div className="mb-4 flex flex-wrap justify-center gap-1.5" aria-label={`Karte ${card + 1} von ${CARD_COUNT}`}>
         ...
         {!(hasAccess || isFreeJourneyStation(activeStation)) ? <PremiumLock title={`${station.era} wartet auf dich`} description={`Du siehst alle ${artPathWithWorks.length} Epochen der Reise. Die ersten ${FREE_JOURNEY_STATIONS} sind frei; Premium öffnet diese und alle folgenden 20-Karten-Stationen.`} /> : <article className="relative min-h-[570px] overflow-hidden rounded-lg border border-border bg-card shadow-sm sm:min-h-[610px]">
          ...
          {card === 1 && <div className="grid min-h-[570px] sm:min-h-[610px] md:grid-cols-[1fr_1fr]">
           ...
           {card === 2 && <div className="flex min-h-[570px] flex-col justify-center p-6 sm:min-h-[610px] sm:p-10"><div className="flex items-center gap-2 text-[10px] tracking-[0.2em] text-muted-foreground uppercase"><ScrollText className="h-4 w-4" /> Zeit & Wendepunkt</div><h3 className="font-display mt-3 text-3xl font-medium">Was die Welt verändert</h3><p className="mt-5 max-w-2xl leading-relaxed text-muted-foreground">{station.atelier?.history}</p><p className="mt-6 max-w-2xl border-l-2 border-foreground pl-5 leading-relaxed">{station.turningPoint}</p><div className="mt-8 grid gap-3 sm:grid-cols-3">{[{ label: "Zeitraum", value: station.years }, { label: "Ort", value: station.place }, { label: "Epoche", value: station.era }].map((fact) => <div key={fact.label} className="rounded-lg border border-border p-4"><p className="text-[10px] tracking-[0.2em] text-muted-foreground uppercase">{fact.label}</p><p className="mt-1 text-sm">{fact.value}</p></div>)}</div></div>}
          ...
          {card === 13 && detailWork && <div className="grid min-h-[570px] sm:min-h-[610px] md:grid-cols-[1fr_1fr]">
           ...
           {card === 16 && <div className="flex min-h-[570px] flex-col justify-center p-6 sm:min-h-[610px] sm:p-10"><div className="flex items-center gap-2 text-[10px] tracking-[0.2em] text-muted-foreground uppercase"><Eye className="h-4 w-4" /> Blickschule & Rückblick</div><h3 className="font-display mt-3 text-3xl font-medium">Was du mitnimmst</h3><div className="mt-6 rounded-lg bg-path-leaf p-6"><p className="leading-relaxed">{station.experience.mission}</p></div><ul className="mt-6 grid gap-3 text-sm leading-relaxed"><li className="rounded-lg border border-border p-4"><span className="font-medium">Epoche:</span> {station.era} ({station.years}) in {station.place}</li><li className="rounded-lg border border-border p-4"><span className="font-medium">Künstler:</span> {station.artists.join(" · ")}</li><li className="rounded-lg border border-border p-4"><span className="font-medium">Materialwissen:</span> {station.atelier?.technique}</li><li className="rounded-lg border border-border p-4"><span className="font-medium">Kernidee:</span> {station.turningPoint}</li></ul></div>}
         ...
         {(hasAccess || isFreeJourneyStation(activeStation)) && <div className="mt-5 flex items-center justify-between gap-3">

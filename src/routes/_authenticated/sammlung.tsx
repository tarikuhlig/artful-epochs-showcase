  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16 md:py-24">
      ...
      <section className="mt-16 border-y border-border py-12">
        ...
        {owned.length === 0 ? <div className="mt-7 rounded-lg bg-coin-soft p-7"><Coins className="h-6 w-6 text-coin" /><p className="mt-3 font-display text-xl">Noch ist die Wand frei.</p><p className="mt-1 text-sm text-muted-foreground">Verdiene Coins auf dem Kunstpfad und ersteigere dein erstes Werk zum Festpreis.</p></div> : <div className="mt-7 grid grid-cols-2 gap-4 md:grid-cols-3">{owned.map((item) => { const work = allWorks.find((candidate) => candidate.id === item.item_slug); if (!work) return null; return <Link key={item.id} to="/werke/$id" params={{ id: work.id }} className="group"><div className="aspect-[4/3] overflow-hidden rounded-md border-[6px] border-coin/50 bg-muted shadow-md"><img src={work.image} alt={work.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" /></div><p className="mt-3 font-medium">{work.title}</p><p className="text-xs text-muted-foreground">{work.painter.name} · {item.purchase_price} Coins</p></Link>; })}</div>}
      </section>
